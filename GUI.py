...
2 SQL TABLES USED -> Reserved (Location, TableNo, ORDERID) [Keeps the currently reserved tables in it] ,
ORDERS (ORDERID, Total, TaxTotal) [Places Order Queries (only customers that reserved a table , with their ORDER ID, Orders can be placed)]
'''
...
''' This rather long dictionary contaning entries for each ORDER ID which is first assigned to 0.
    Once user with that ORDER ID places an order, the value is replaced with the total of their order 
